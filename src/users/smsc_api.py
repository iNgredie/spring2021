...
SMSC_LOGIN = settings.MY_SMSC_ACCOUNT  # логин клиента
SMSC_PASSWORD = settings.MY_SMSC_PASSWORD  # пароль
...
class SMSCException(Exception):
    """
    Базовое исключение для работы с SMSC
    """
    def __init__(self, msg_id, error_id):
        self.msg_id = int(msg_id)
        self.error_id = abs(int(error_id))

    def __str__(self):
        url = r'https://smsc.ru/api/http/send/sms/sms_answer/'
        msg = (
            f'Ошибка {self.error_id} для сообщения {self.msg_id}\n',
            f'Подробнее об ошибке: {url}'
        )
        return repr(msg)
